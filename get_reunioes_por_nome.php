...
if (isset($_GET['userName'])) {
    ...
    // Consulta SQL para selecionar as reuniões em que o nome do usuário está presente na coluna 'participantes'
    $query = "SELECT * FROM reunioes WHERE participantes LIKE :userName";
    $stmt = $conn->prepare($query);
    $stmt->execute(array(':userName' => "%$userName%")); // Use % para corresponder a qualquer parte do nome
    $reuniaoData = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Agora você tem os dados da reunião para o usuário clicado na variável $reuniaoData

    // Envie os dados de volta para o cliente (JavaScript) para serem usados na renderização do gráfico
    echo json_encode($reuniaoData);
} else {
